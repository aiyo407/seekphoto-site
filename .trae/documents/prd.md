...
- **主色调**：深色背景（`#0f1419` → `#0a0e13`），卡片背景 `#1a1f2e`
- **强调色**：青蓝色 `#00d9ff`，深青蓝 `#0099cc`
- **文字色**：主文字白色 `#ffffff`，次要文字 `#8b95a5`
- **边框色**：`#2a3040`
- **字体**：
  - 中文正文：Noto Sans SC
  - 英文/标题：Outfit
- **按钮风格**：圆角 12px、渐变背景、悬停上浮 + 阴影扩散
- **布局风格**：单页多区块、卡片式布局、居中大容器
- **图标风格**：内联 SVG 线性图标，1.5-2px 描边

### 4.2 页面设计概览

| 页面名称 | 模块名称 | UI 元素 |
|----------|----------|---------|
| 首页 | Hero 区域 | 深色渐变背景、网格纹理、光晕效果、徽章、大标题、副标题、CTA 按钮 |
| 首页 | 核心特性 | 三列网格卡片、图标、标题、描述、示例标签 |
| 首页 | 搜索演示 | 搜索框 + 6 个渐变色结果方块 + 匹配度百分比 |
| 首页 | 隐私承诺 | 大卡片、锁图标、三个标签胶囊 |
| 首页 | CTA | 大按钮 + 版本说明文字 |
...
- **平板适配**：968px 以下功能详情改为单列，特性网格改为 2 列
- **移动端**：768px 以下 Hero 标题缩小、特性单列、下载步骤纵向排列、页脚单列

### 4.4 动效设计

- **Hero 区域**：内容淡入上浮（`fadeInUp`），按徽章 → 标题 → 副标题 → 按钮依次延迟 0.2s
- **背景**：网格纹理 + 两个模糊光晕（无粒子系统）
- **搜索演示**：打字机效果循环切换 5 个关键词，光标闪烁
- **结果卡片**：`fadeIn` 动画，按序号递增延迟
- **卡片悬停**：上移 4px、边框变为主色、阴影加深
